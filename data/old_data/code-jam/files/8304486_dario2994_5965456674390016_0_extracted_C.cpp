...
bool used[MAXC][2];
int RecursiveVisit(int c, int length, int total_time) {
    // cout << c << " " << length << " " << total_time << endl;
    if (length == 2*C) return total_time;
    int res = 1e9;
    for (int i = 0; i < 2; i++) {
        if (used[c][i]) continue;
        used[c][i] = true;
        int waiting = total_time + dd[c][i] + ll[c][i] - (total_time % 24);
        if (total_time % 24 > ll[c][i]) waiting +=  24;
        MinPlace(res, RecursiveVisit(aa[c][i], length+1, waiting));
        used[c][i] = false;
    }
...
void SolveProblem(ifstream& in, ofstream& out) {
    in >> C;
    for (int i = 0; i < 2*C; i++) {
        in >> aa[i/2][i%2] >> ll[i/2][i%2] >> dd[i/2][i%2];
        aa[i/2][i%2]--;
    }

    int res = RecursiveVisit(0, 0, 0);
    
    out << res << "\n";
}

const bool SET_THIS_TO_TRUE_FOR_OFFICIAL_SUBMISSION = true;
const string official_input_name = "C-small-attempt0.in";
