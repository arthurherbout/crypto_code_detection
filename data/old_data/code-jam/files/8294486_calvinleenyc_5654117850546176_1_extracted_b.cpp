...
void solve(){
  ...
  if(b + o == n){
    if(b != o) cout << "IMPOSSIBLE";
    else{ for(int i=0;i<b;i++) cout << "BO"; }
    return;
  }
  if(r + g == n){
    if(r != g) cout << "IMPOSSIBLE";
    else{ for(int i=0;i<r;i++) cout << "RG"; }
    return;
  }
  if(y + v == n){
    if(y != v) cout << "IMPOSSIBLE";
    else{ for(int i=0;i<y;i++) cout << "YV"; }
    return;
  }
  
  if(1+ o > b || 1+ g > r || 1+ v > y){
    cout << "IMPOSSIBLE";
    return;
  }
  b -= o;
