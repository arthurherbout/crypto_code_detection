...
map<VS,bool> memo[8][8];

bool wins(VS board,int r,int c){
	assert( board[r][c] != '#' );
	if( memo[r][c].count(board) ) return memo[r][c][board];
	bool& ret = memo[r][c][board];
	REP(d,8){
		int r2 = r + DR[d], c2 = c + DC[d];
		if( IN(0,r2,R-1) && IN(0,c2,C-1) && board[r2][c2] != '#' ){
			board[r][c] = '#';
			if( !wins(board,r2,c2) ) return ret = true;
			board[r][c] = '.';
		}
	}
	return ret = false;
...
void run(int tc){
	...
	REP(r,R) REP(c,C) memo[r][c].clear();
	cout << "Case #" << tc << ": " << (wins(board,sr,sc) ? 'A' : 'B') << endl;
}
int main(){
	int TC;
	cin >> TC;
