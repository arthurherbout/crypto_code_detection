...
bool check(int &key,int &c1,int &c2,int &c3)
{
	if (key==1)
	{
...
int solve(int c,int c1,int c2,int c3,int n)
{
	if (n==0) return 1;
	int &ret=f[c-1][c1][c2][c3][n];
	if (ret>=0)
		return ret;
	for (int key=1;key<=c;key++)
	{
		int s1=c1,s2=c2,s3=c3;
		if (!check(key,s1,s2,s3))
			return ret=0;
		if (!solve(c,s1,s2,s3,n-1))
			return ret=0;
	}
	return ret=1;
}
