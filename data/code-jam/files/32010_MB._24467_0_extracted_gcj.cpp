...
int main(){
	...
	FUP(tt,1,TT){
		...
		int ret = 10000;
		FUP(i,0,N-1){
			string ts = io.readString();
			if (colors.count(ts)==0)
				colors[ts] = colors.size();
			off[i].second = colors[ts];
			off[i].first.first = RI;
			off[i].first.second = RI;
		}
		C = colors.size();
		sort(off,off+N);
		FUP(i,0,C-1)FUP(j,0,C-1)FUP(k,0,C-1){
			int nx = 1;
			int best = 0;
			int sret = 0;
			FUP(a,0,N-1){
				if (off[a].first.first > nx){
					if (best == 0)
						break;
					nx = best+1;
					sret++;
				...
				if (off[a].first.first <= nx && 
					(off[a].second == i ||
						off[a].second == j||
						off[a].second == k))
					maxn(best, off[a].first.second);
			}
			if (nx < 10001 && best >= 10000){
				sret++;
				nx = best+1;
			}
			if (nx == 10001)
		...
		if (ret < 10000){
			printf("Case #%d: %d", tt, ret);
			printf("\n");
		}else{
			printf("Case #%d: IMPOSSIBLE", tt);
			printf("\n");
		}
	}
	return 0;
