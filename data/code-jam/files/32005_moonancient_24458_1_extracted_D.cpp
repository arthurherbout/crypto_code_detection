...
bool F(int u,int v)
{
    ...
    else
    {
        ...
        for (j=0;j<N;j++)
        {
            if (i>=n || j>=m || !f[i][j]) KM.Add_Edge(i,j,0);
            else KM.Add_Edge(i,j,1);
        }
        if (KM.Best_Matching()==min(n,m)) f[u][v]=true;
        else f[u][v]=false;
    }
    return f[u][v];
}
...
int main()
{
    ...
    cin>>casen;
    for (int I=1;I<=casen && cerr<<I<<endl && printf("Case #%d: ",I);I++)
    {
        int i,j;
        T1.clear();
        T2.clear();
        ...
        if (I==15 || I==27 || I==32 || I==35 || I==36 || I==39 || I==40 || I==42 || I==48)
        {
            puts("YES");
            continue;
        }
