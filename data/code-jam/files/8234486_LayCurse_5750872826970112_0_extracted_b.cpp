...
int main(){
  ...
  while(T--){
    ...
    reader(&N,&V,&X);
    rep(i,N) reader(R+i,C+i);
    rep(i,N) C[i] -= X;
    sort(N, C, R, mem);
    if(C[0] > EPS || C[N-1] < -EPS){ writerLn("IMPOSSIBLE"); continue; }

    sp = 0;
    st = -1;
    ed = N;
    rep(i,N) if(C[i] < -EPS) st=i;
    for(i=N-1;i>=0;i--) if(C[i] > EPS) ed=i;
    rep(i,N) if(-EPS < C[i] && C[i] < EPS) sp += R[i], R[i] = 0;

    rep(i,N) if(C[i] < 0) C[i] = -C[i];
    ...
    for(;;){
      ...
      rep(loop,300){
        ...
        a = zz * C[ed];
        b = zz * C[st];
        if(a > R[st] || b > R[ed]) yy = zz; else xx = zz;
      }
      sp += a + b;
      R[st] -= a;
      R[ed] -= b;
