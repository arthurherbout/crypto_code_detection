...
int main(){
    ...
    Rush{
        ...
        RST(sz); ECH(it, root) dfs(*it, *it);

        REP(i, n) RC(title[i]);
        RD(m); REP(i, m){
            Pn[i] = strlen(RS(P[i]));
            KMP::get_pi(P[i], Pn[i], pi[i]);
        }
        ...
        int _T = 10000;
        DO(_T){
            Tn = 0; cur = root; VI tmp;

            while (!cur.empty()){


                tmp.clear(); ECH(it, cur){
                    DO(sz[*it]) tmp.push_back(*it);
                }

//                cout << cur.size() << " " << tmp.size() << endl;

                int t = rand() % tmp.size(); //rand() % cur.size();
              //  cout << t << " " << cur[t] << endl;
                int u = tmp[t]; tmp.clear(); ECH(it, cur) if (*it != u) tmp.push_back(*it);
                 cur = tmp;
                gao(u);
            }
