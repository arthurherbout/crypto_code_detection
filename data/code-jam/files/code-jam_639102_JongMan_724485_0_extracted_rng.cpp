...
{
    long long up, down;
    Frac(long long u = 0, long long d = 1) : up(u), down(d) { long long g = gcd(up, down); up /= g; down /= g; }
    Frac inv() const { return Frac(down, up); }
...
void solve() {
    ...
    for(int i = 0; i < primes.size(); ++i) {
        ...
        printf("mod = %d\n", MOD);
        REP(i,matrix.sz) {
            REP(j,matrix[i].sz) printf("%-4d", matrix[i][j]);
            printf("\n");
        }*/
        if(matrix.sz <= 1) {
            printf("I don't know.\n");
