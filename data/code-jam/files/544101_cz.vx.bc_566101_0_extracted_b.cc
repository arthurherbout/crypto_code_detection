...
int main() {
  ...
  while (nt-- && cin >> dc >> ic >> md >> n) {
    ...
    q.insert(make_pair(0, make_pair(256, 0)));
    while (!q.empty()) {
      int d = q.begin()->first;
	...
	continue;
      ...
      q.insert(make_pair(d+dc, make_pair(pc, p+1)));
      
      if (pc != 256) {
	for (int i = pc-md; i <= pc+md; ++i)
	  if (i >= 0 && i < 256 && dist[i][p] > d+ic)
	    q.insert(make_pair(d+ic, make_pair(i, p)));

	for (int i = pc-md; i <= pc+md; ++i)
	  if (i >= 0 && i < 256)
	    q.insert(make_pair(d+abs(A[p]-i), make_pair(i, p+1)));
      } else {
	for (int i = 0; i <= 255; ++i)
	  q.insert(make_pair(d+abs(A[p]-i), make_pair(i, p+1)));
      }
