...
int N;
pnt p[4000]; int res[3300];

double th[10000]; int sz;
double pi;
...
int main(){
  ...
  while(T--){
    ...
    rep(i,N){
      reader(x+i,y+i);
      p[i]=pntGenerator(x[i],y[i]);
    }
    rep(i,N) res[i] = N;
    if(N<=3) rep(i,N) res[i] = 0;
    
    rep(i,N){
      sz = 0;
      rep(j,N) if(j!=i){
        th[sz++] = atan2(y[j]-y[i],x[j]-x[i]);
        th[sz] = th[sz-1] + 2*pi; sz++;
        th[sz] = th[sz-1] + 2*pi; sz++;
      }
      sort(th, th+sz);
      k = 0;
      rep(j,N){
        if(th[j] > 2*pi) break;
        while(th[k] < th[j]+pi-EPS) k++;
        res[i] = min(res[i], k-j-1);
      }
    }
